...
class SineCurveUnitCircle(Scene):
    def construct(self):
        ...
        def get_line_to_circle():
            point=orbit.point_from_proportion(tracker.get_value())
            newdot.move_to(point)
            seg=Line(np.array([-4,0,0]),point, color=GREEN_B,
        ...
        #He necesitado instanciar un punto para que el segmento vaya desde el centro 
        #al punto de órbita 
        newdot = Dot(radius=0.8, color=RED)
        self.newdot=newdot
        ...
        self.anima_segmentos(0, np.pi/2,  10, 0.4)
        ...
        self.anima_segmentos(np.pi/2, np.pi,  10, 0.3)
        ...
        self.anima_segmentos(np.pi, 2*np.pi,  20, 0.3)
